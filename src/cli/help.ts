...
export class HelpSystem {
  ...
  /** Tool description */
  private description = "A minimalist test runner for TypeScript";

  /** Current tool version */
  private version = "1.0.0";

  /** Usage string displayed in the instructions section */
  private usage = "muitto [options] [files...]";
  ...
  render(): string {
    ...
    const maxFlagLen = Math.max(
      ...this.options.map((opt) => {
        ...
        if (opt.alias) flags.push(opt.alias);
        if (opt.value) flags.push(`<${opt.value}>`);
    ...
    for (const option of this.options) {
      ...
      if (option.value) flags.push(`<${option.value}>`);
 ...
 * help.addOptions(predefinedOptions);
 */
export const predefinedOptions: CliOption[] = [
  {
    flag: "--watch",
    alias: "-w",
