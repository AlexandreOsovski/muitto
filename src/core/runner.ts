 ...
 * });
 */
export async function runTests(options: RunOptions = {}): Promise<RunSummary> {
  ...
  if (files.length === 0) {
    ...
    config.pattern.forEach((p: string) => console.log(`  - ${p}`));
    process.exit(0);
  }
  ...
  for (const file of files) {
    ...
    if (options.bail && result.results.some((r: TestResult) => r.status === "failed")) {
      break;
    }
  }
...
async function executeTests(
  tests: TestCase[],
  hooksBySuiteKey: Map<string, any>,
  options: {
    timeoutMs: number;
    filter: string;
    reporter: Reporter;
  ...
  const hasOnly = tests.some((t: TestCase) => t.only);
  const results: TestResult[] = [];

  /** Controls which suites have already had beforeAll executed */
  const suitesRun = new Set<string>();
  ...
  for (const test of tests) {
    ...
    if (hooks && !suitesRun.has(suiteKey)) {
      ...
      try {
        ...
        if (options.bail) return results;
        continue;
      }
    }
    ...
    try {
      ...
      if (hooks) await runHookChain(hooks.beforeEach);
      await withTimeout(test.fn, options.timeoutMs);
      if (hooks) await runHookChain(hooks.afterEach);
