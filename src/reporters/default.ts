...
export class DefaultReporter extends BaseReporter {
    ...
    if (failed.length > 0) {
      ...
      for (const failure of failed) {
        console.log(
          formatTestFailure(
            failure.test.name,
            failure.test.suite.join(" > "),
            failure.error,
            this.relativePath(failure.test.filePath)
          )
        );
      }
    }
  ...
  onEnd(summary: RunSummary): void {
    ...
    console.log(
      color.dim(
        `Tests       ${totalPassed} passed` +
        (totalSkipped > 0 ? ` | ${totalSkipped} skipped` : "") +
        (totalFailed > 0 ? ` | ${totalFailed} failed` : "")
      )
    );
    console.log(
      color.dim(`Duration    ${this.formatDuration(summary.durationMs)}`)
    );
   ...
   * @private
   */
  private getIcon(status: "passed" | "failed" | "skipped"): string {
    switch (status) {
      case "passed":
        return symbols.pass;
      case "failed":
