...
export class JunitReporter extends BaseReporter {
    ...
    buildXml(summary) {
        ...
        lines.push(`<testsuites name="${this.escape(suiteName)}" tests="${total}" ` +
    ...
    buildFileSuite(file, timestamp) {
        ...
        lines.push(`  <testsuite name="${this.escape(file.filePath)}" tests="${testsAttr}" ` +
        ...
        if (file.collectError) {
            ...
            lines.push(`    <testcase name="collect error" classname="${this.escape(file.filePath)}">`);
