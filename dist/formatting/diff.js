 ...
 */
export function generateDiff(expected, received, path = "") {
    const lines = [];
    ...
    if (typeof expected === "object" && typeof received === "object") {
        return generateObjectDiff(expected, received, path);
    }
 ...
 */
function generateArrayDiff(expected, received, path) {
    const lines = [];
    ...
    for (let i = 0; i < maxLen; i++) {
        ...
        if (i >= expected.length) {
            ...
            lines.push(...parseDiffLines(generateDiff(expected[i], received[i], currentPath)));
        }
 ...
 */
function generateObjectDiff(expected, received, path) {
    const lines = [];
    // Collects and sorts all keys from both objects
    const allKeys = Array.from(new Set([...Object.keys(expected), ...Object.keys(received)])).sort();
    for (const key of allKeys) {
        ...
        if (!(key in expected)) {
            ...
            lines.push(...parseDiffLines(generateDiff(expectedVal, receivedVal, currentPath)));
        }
...
function parseDiffLines(diff) {
    ...
    return diff
        ...
        if (line.startsWith("- "))
            return { type: "removed", value: line };
        return { type: "unchanged", value: line };
    });
}
/**
